...
VALUES = db.find('2023', "trigonometria", int(random.random()))
print(f"\n\t{VALUES}\n")
displayImage(canvas)
...
botao_A.configure(bg="yellow", font="Times 24 bold", command=validateA(canvas))
botao_A.pack(side=LEFT, padx=10, pady=10)

botao_B =Button(window, text="time B", width=10, height=5)
botao_B.configure(bg="blue", font="Times 24 bold", command=validateB(canvas))
